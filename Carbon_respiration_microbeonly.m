...
%% Parameters
k_subC=0.009/24/12;%Empirical factor of decomposition rate, labile                1/hour
...
k_CB1=30000;%Carbon usage at 50% of the maximum rate U, fast           gC/m3 water diluted
k_CB2=10000;%Carbon usage at 50% of the maximum rate U, slow           gC/m3 water diluted
...
solC=totC*0.008;%soluble carbon fast           gC/m3 water diluted
...
for i=1:length(s)
    ...
    U1(i)=k_U1(i)*C_B1(i)*solC(i)^n/(solC(i)^n+k_CB1^n);
    U2(i)=k_U2(i)*C_B2(i)*solC(i)^n/(solC(i)^n+k_CB2^n);
    
    R_solC(i)=k_RsolC*solC(i);
    subC(i+1)=subC(i)+litterFall-D(i);
    solC(i+1)=solC(i)+D(i)-U1(i)-U2(i)-R_solC(i)+M_BD*C_BD1(i)+M_B*C_B1(i);
    ...
    T_resp(i)=f_e*(U1(i)+U2(i)+U21(i)+U22(i))+R_solC(i);
end
...
plot(1:length(s),T_resp*2.7/1.51872*s(1),'k-','LineWidth',1);
